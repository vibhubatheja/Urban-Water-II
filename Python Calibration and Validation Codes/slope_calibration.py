...
#minwid=z[2]
 maxslp=z[5]+1
 minslp=z[4]
 steps=(maxslp-minslp)/3


 for slope in np.arange (minslp,maxslp,steps) :   ## in range (start,stop+1,step)
   print((slope/maxslp)*100,"%")  
   inp.SUBCATCHMENTS[str(j)].slope=slope
   ...
   if nse>bestNSE :
      bestslp=slope
      bestNSE=nse
      print('change made NSE',bestNSE,'best slope',bestslp)


############### Reset to best values and change best values if not better NSE compare once more outside loop 
      
 inp.SUBCATCHMENTS[str(j)].slope=bestslp
 inp.write_file('P_Tud_Sewer_V2.inp')

 print('THE BEST VALUE OF NSE SO FAR',bestNSE)
 print('THE BEST VALUE OF SLOPE for Subcatchment  ',j,'is',bestslp)
