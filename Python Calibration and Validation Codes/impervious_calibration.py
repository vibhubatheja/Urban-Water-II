...
bestNSE=0.84     ################################################## Best NSE change if it shows something good   ENTER MANUALLY 
...
for j in (rangeofvalues):
 ...
 bestwid=(inp.SUBCATCHMENTS[str(j)].width) # current width   of only this subcatchment  use function
 bestimp=(inp.SUBCATCHMENTS[str(j)].imperviousness)   #current imp   of only this subcatchment  use function 
 bestslp=inp.SUBCATCHMENTS[str(j)].slope # current slope
 ...
 maximp=z[1]+1
 minimp=z[0]
 steps=(maximp-minimp)/10




 for imperv in np.arange (minimp,maximp,steps) :   ## in range (start,stop+1,step)
   print((imperv/maximp)*100,"%")  
   inp.SUBCATCHMENTS[str(j)].imperviousness=imperv
   ...
   if nse>bestNSE :
      bestimp=imperv
      bestNSE=nse
      print("change best NSE for subcatchment",j)


############### Reset to best values and change best values if not better NSE compare once more outside loop 
      
 inp.SUBCATCHMENTS[str(j)].imperviousness=bestimp
 inp.write_file('P_Tud_Sewer_V2.inp')
 #print('THE BEST VALUE OF NSE SO FAR',bestNSE)
 print('THE BEST VALUE OF IMPERVIOUSNESS for Subcatchment  ',j,'is',bestimp,'the NSE is',bestNSE)
      



#for slope in 
 #inp.SUBCATCHMENTS['1'].slope=slope  
